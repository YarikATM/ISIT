...
def clean_text(texts_numbers):
    ...
    """
    if texts_numbers is None or len(texts_numbers) == 0:
        return None
    
    filter_texts = []
    for text in texts_numbers:
        if len(text) in [6, 8, 9]: 
            new_text = ""
...
"""
Это основная функция, по обработке как изображения, так и видео
"""
...
"""Функция, которая должна срабатываеться, если пользователь скинул изображение"""
def image_detection_and_recognition(model_yolo, img):
    # img = cv2.imread(image)
    img_mod = model_yolo(img)
    
    cars_numbers = {}
    cars_numbers = license_plate_detection_and_recognition(img, img_mod)

    return cars_numbers


"""Функция, которая должна срабатываеться, если пользователь скинул видео"""
...
"""
Фильтруем, чтобы не попались два одинаковых номера
"""
def filter_unique_numbers(dict_num):
    unique_numbers = [] # можно её возварщать в качетсве наёденных номеров и в боте выводить этот список
    if dict_num is None or len(dict_num) == 0:
        return None

    for values in dict_num.values():
        for value in values:
...
def recognize(img):
    model = YOLO("api/ai/runs/detect/train3/weights/best.pt")

    numbers = image_detection_and_recognition(model, img)
    u_numbers = filter_unique_numbers(numbers)
    return u_numbers


def main(image_path):
    model = YOLO("runs/detect/train3/weights/best.pt")
    numbers = image_detection_and_recognition(model, image_path)
    u_numbers = filter_unique_numbers(numbers)
    print(u_numbers)


if __name__ == "__main__":
      # Путь до нашей модели Yolo


    # сценарий 1 (Получаем изображение от пользователя)
    main()

    # # сценарий 2 (Получаем видео от пользователя)
    # video_path = "путь/до/видео"
    # numbers = image_detection_and_recognition(model, video_path)
    # u_numbers = filter_unique_numbers(numbers)
    # output_numbers(u_numbers)
